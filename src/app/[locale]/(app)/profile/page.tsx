...
import { User, Lock, Trash2, Check, AlertTriangle, Loader2 } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const isGoogleOnly = user?.avatarUrl && !user?.email?.includes('local_')
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {user?.role !== 'SUPER_ADMIN' && (
        <div className="rounded-xl border border-danger/30 bg-bg-card p-6">
          <div className="mb-4 flex items-center gap-2">
            <Trash2 className="h-5 w-5 text-danger" />
            <h2 className="text-lg font-semibold text-danger">{t('delete_account')}</h2>
          </div>

          <p className="mb-4 text-sm text-text-muted">{t('delete_account_desc')}</p>

          <form onSubmit={handleDeleteAccount} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">{t('delete_confirm')}</label>
              <input
                type="password"
                value={deletePassword}
                onChange={(e) => setDeletePassword(e.target.value)}
                className="w-full rounded-lg border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary focus:border-danger focus:ring-1 focus:ring-danger"
                required
              />
            </div>
